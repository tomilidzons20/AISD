...
class Graph:
    ...
    def show(self, path = Optional[List[Vertex]]) -> None:
        G = nx.DiGraph()
        # set edges
        edges = []
        for key, value in self.adjacencies.items():
    ...
    while v:
        ...
        for key, value in g.adjacencies.items():
            if key == v:
                for n in value:
                    nc = c + n.weight
                    # if cheaper path is found change path
                    if price[n.destination] > nc:
                        # update price
                        price.update({n.destination: nc})
                        # clear path
                        paths[n.destination].clear()
                        # if vertex is visited remove from visited list
                        if n.destination in visited1:
                            visited1.remove(n.destination)
                        # add to path current vertex
                        paths[n.destination].append(v)
                    # if next vertex is not in visited then add it to path
                    if n.destination not in visited1:
                        paths[n.destination].append(n.destination)
                    # add next vertex to path
                    visited1.append(n.destination)
        visited.append(v)
        v = closest_vertex()
    # completion of paths (most likely to complete beginning of path)
    ils = 0
    # while there is at least one path that does not start from starting vertex
    while ils != len(paths):
        ils = 0
...
v4 = graf.create_vertex("v4")
v5 = graf.create_vertex("v5")
graf.add(EdgeType.directed, v0, v1, 1)
graf.add(EdgeType.directed, v0, v2, 2)
graf.add(EdgeType.directed, v2, v1, 4)
graf.add(EdgeType.directed, v2, v4, 3)
graf.add(EdgeType.directed, v1, v3, 1)
graf.add(EdgeType.directed, v3, v4, 1)
graf.add(EdgeType.directed, v3, v5, 2)
graf.add(EdgeType.directed, v4, v5, 1)
...
# graf.add(EdgeType.directed, v0, v1, 2)
# graf.add(EdgeType.directed, v0, v3, 4)
# graf.add(EdgeType.directed, v1, v3, 3)
# graf.add(EdgeType.directed, v1, v2, 3)
# graf.add(EdgeType.directed, v2, v4, 2)
# graf.add(EdgeType.directed, v3, v2, 3)
# graf.add(EdgeType.directed, v3, v4, 4)
...
# graf.add(EdgeType.directed, v0, v1, 3)
# graf.add(EdgeType.directed, v0, v3, 3)
# graf.add(EdgeType.directed, v1, v2, 2)
# graf.add(EdgeType.directed, v2, v5, 1)
# graf.add(EdgeType.directed, v2, v0, 6)
# graf.add(EdgeType.directed, v3, v4, 1)
# graf.add(EdgeType.directed, v4, v2, 1)
# graf.add(EdgeType.directed, v4, v5, 2)
# graf.add(EdgeType.directed, v5, v3, 3)
...
# graf.add(EdgeType.directed, v0, v1, 1)
# graf.add(EdgeType.directed, v0, v11, 2)
# graf.add(EdgeType.directed, v11, v10, 3)
# graf.add(EdgeType.directed, v10, v9, 2)
# graf.add(EdgeType.directed, v9, v8, 3)
# graf.add(EdgeType.directed, v8, v7, 2)
# graf.add(EdgeType.directed, v7, v6, 3)
# graf.add(EdgeType.directed, v1, v2, 3)
# graf.add(EdgeType.directed, v2, v3, 5)
# graf.add(EdgeType.directed, v3, v4, 1)
# graf.add(EdgeType.directed, v4, v5, 1)
# graf.add(EdgeType.directed, v5, v6, 1)
