            ...
            if key == v:
                for n in value:
                    ...
                    if price[n.destination] > nc:
                        ...
                        # if next vertex in visited then remove it from visited to change path then add edge to path
                        if n.destination.data in visited1:
                            visited1.remove(n.destination.data)
                        paths[n.destination.data].append(n)
                    # add edge to path if not visited and not already in path
                    if n.destination.data not in visited1 and n not in paths[n.destination.data]:
                        paths[n.destination.data].append(n)
