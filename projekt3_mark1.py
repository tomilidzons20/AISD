...
def all_weighted_shortest_paths(g: Graph, start: Any) -> Dict[Any, List[Edge]]:
    ...
    while v:
        ...
        visited.append(v)
        v = closest_vertex()
    # create paths dictionary and complete paths
    paths: Dict[Any, List[Edge]] = {}
    for key in g.adjacencies.keys():
        paths.update({key.data: []})
    # creating initial paths dictionary using parents dictionary
    paths[start1.data].append(Edge(start1, start1, 0))
    for key, value in parents.items():
        edge = None
        for i in g.adjacencies[value]:
            if i.source is value and i.destination is key:
                edge = i
        paths[key.data].append(edge)
    # complete paths
    hlp = 0
    while hlp != len(paths):
        hlp = 0
        for k, v in paths.items():
            if len(v) != 0 and v[0].source.data is not start1.data:
